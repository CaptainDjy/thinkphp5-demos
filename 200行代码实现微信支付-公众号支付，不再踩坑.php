附demo:
开发微信支付需要认证服务号并且已经开通微信支付，获得微信支付商ID，设置apikey了的。

官方给了公众号支付的php源码SDK

但是这个SDK文件非常多，源码乱七八糟的，真的一开始接触的人拿着官方的SDK肯定踩坑。

而且还不知道最后能不能用。

我看到了一个作者写的源码，还不错，我简单进行优化了，只有200多行代码就可以实现公众号支付。

在开发之前，得先配置网页授权的域名和支付授权域名。网页授权域名在公众号后台，接口权限找到网页获取用户基本信息处配置你的代码所在的目录，例如你的公众号支付代码放在www.baidu.com/pay/jspai.php

那么网页授权域名为：www.baidu.com/pay/

支付授权域名配置要登录商户平台

进入微信商户号－－产品中心－－开发配置，然后往下拉找到公众号支付，添加授权目录即可，授权目录也是你的代码所在目录．

微信支付

然后往下拉找到公众号支付

添加授权目录即可，授权目录也是你的代码所在目录。
...
简单说明：
1】新建jsapi.php，拷贝上面的代码，修改商户号、appid、appsecret、密钥即可，还有下面的回调地址，前面的域名需要换成你的nofity.php的地址。至于支付成功和支付失败跳转的页面按自己的需求修改即可。
要注意，订单标题不能为中文，否则会提示body参数错误什么的。
2】新建nofity.php
把上面的代码拷贝上去，修改商户号、appid、密钥即可。
上传到支付授权目录，然后访问jsapi.php页面测试。
