...
class Wechat
{
    private $redirect_url = "http://www.yhpkj.com/api/wechat/UserInfo";//这里填你自己的地址！！要能访问到这个类下的getOpenid方法！！  改
//用户同意授权，获取openid
//应用授权作用域，snsapi_base （不弹出授权页面，直接跳转，只能获取用户openid）,  snsapi_userinfo （弹出授权页面，可通过openid拿到昵称、性别、所在地。）
    public function UserInfo($scope = 'snsapi_userinfo')
    {
        if (!isset($_GET['code'])) {
...
//获取code码用以获取openid
            ...
            $data = $this->getUserInfo(json_decode($res, true));
          $member=new Member();
          $result=$member->login_check($data);
            //return $data;
            //return $res;//完整数据
//return $res['openid'];
        }
    ...
    {
        $appid = 'wxdd8979589794a59e';//微信公众号appid    改
        //$state = $_GET['id'];//状态码
//处理回调域名
...
//拼接url
    ...
    {
        $appid = 'wxdd8979589794a59e';//微信公众号appid  改
        $secret = 'd9b6aaf3a561f1a11338941968027555';//应用密钥  改
